...
```bash
matugen image ~/Pictures/your-wallpaper.jpg   # regenerates all 42 outputs live
```
...
| `firefox` (+ chrome wiring) | `templates/firefox.css` (userChrome, UI only) + `templates/firefox-content.css` (new-tab page) | `theme/current/firefox{,-usercontent}.css` → symlinked as `userChrome.css`/`userContent.css` in default profile; `firefox/user.js` enables it |
| `pear-desktop` | `templates/pear-desktop.css` | `snow_black` + `current` |
| `rofi` / `neovim` / `swayosd` / `zed` / `obsidian` / `swaync` | respective templates | per-app `theme/current` outputs |
...
`SUPER+K` opens the searchable cheatsheet (same list, executable). Essentials:

| Keys | Action |
|------|--------|
| `SUPER+Return` / `SUPER+Space` | Terminal / launcher |
| `SUPER+SHIFT+F` / `B` / `N` | Filemanager / browser / Zed |
| `SUPER+E` / `I` / `ALT+N` | Theme / packages / PDF library |
| `SUPER+R` / `Print` | Record toggle / screenshot |
| `SUPER+W` / `T` / `F` | Close / float / fullscreen |
| `SUPER+arrows` / `SUPER+1–0` | Focus / workspaces |
| `SUPER+,` / `SHIFT+,` | Notifications / do-not-disturb |

Full list lives in `.config/hypr/bindings.conf` (+ vendored defaults in `.config/hypr/vendor/`).
...
│   ├── matugen/
│   │   ├── config.toml       # 40+ templates
│   │   └── templates/        # 40+ per-app templates (btop, cava, gtk, walker, rofi, nvim, zed, sddm, zathura, …)
